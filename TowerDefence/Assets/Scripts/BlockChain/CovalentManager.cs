...
public class CovalentManager : MonoBehaviour
{
                ...
                case UnityWebRequest.Result.Success:
                    ...
                    if (_data.GetField("data").HasField("items"))
                    {
                        ...
                        for (int i = 0; i < _data.GetField("data").GetField("items").list.Count; i++)
                        {
                            ...
                            if (MoralisManager.contract.ToLower().Equals(_add))
                            {
    ...
    {
        using (UnityWebRequest webRequest = UnityWebRequest.Get(GetMetaDataPreURL + MoralisManager.contract + GetMetaDataMidURL + _tokenid + GetMetaDataPostURL))
        {
